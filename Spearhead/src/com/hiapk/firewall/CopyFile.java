...
public class CopyFile {
	...
	public static boolean hasRootAccess(Context context, boolean showErrors) {
		...
		boolean hasRoot = sharedData.isHasRoot();
		if (hasRoot)
			return true;
		final StringBuilder res = new StringBuilder();
		try {
			if (runScript(context, "exit 0", res, 20000, true) == 0) {
				sharedData.setHasRoot(true);
	...
	public static boolean assertBinaries(Context ctx, boolean showErrors) {
		...
		try {
			...
			if (changed) {
				Toast.makeText(ctx, "防火墙已安装", Toast.LENGTH_LONG).show();
			}
		} catch (Exception e) {
			if (showErrors)
